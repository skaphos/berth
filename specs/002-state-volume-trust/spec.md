...
within a bounded time derived from the lease TTL, and that it does not fail
while the sidecar is renewing normally — including across a renew that fails
transiently but within the TTL.
...
- **First-write race.** The init container gates startup, but the marker is
  written by the renew loop; a freshness bound must not kill a container in
  the window before the first renewal.
- **Legitimately slow renewals.** The bound must tolerate the worst-case gap
  between two successful renewals under a heartbeat that is, by validation,
  strictly less than the TTL — otherwise a healthy holder kills itself.
...
- **FR-006**: The liveness check MUST fail when the health marker has not been
  refreshed within a freshness bound derived from the lease TTL, in addition
  to failing when it is absent. (#98)
- **FR-007**: The freshness bound MUST NOT fail a holder whose sidecar is
  ...
  gap between two successful renewals permitted by configuration.
- **FR-008**: The freshness evaluation MUST NOT depend on agreement between
  the sidecar's clock and the main container's clock.
...
- **FR-012**: Documentation MUST describe the delivered behavior: the state
  ...
  tests freshness as well as presence; and the upgrade is breaking, with
  guidance for finding affected workloads beforehand.
...
- **Freshness bound**: the maximum age of a marker that still counts as
  healthy, derived from the lease TTL, and conveyed to `check` without new
...
- **SC-001**: No pod spec admits that leaves the health marker or `check`
  ...
  test matrix spanning mount path, `readOnly`, container kind, and both
  enforce modes.
- **SC-002**: With the sidecar stopped and the state volume untouched, the
  liveness check begins failing within the freshness bound; the workload is
  killed without any action by the sidecar.
...
- `enforce: signal` shares the state volume for holder/token handoff but does
  not depend on the marker; the US1 mount rule applies to both modes, while
  the US2 freshness rule is specific to `probe`.
