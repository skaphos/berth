...
| `berth-operator` | `metrics.bindAddress` | `:<port>`, `<host>:<port>`, `[<ipv6>]:<port>`, a bare `<port>` (string or integer), or `0` / `""` to disable the listener, which also omits the metrics container port. Ports are normalized to a decimal integer. Anything else, including URLs and unbracketed IPv6, fails rendering. |
